...
type (
	...
	ShortUrlMap struct {
		...
		LurlHash sql.NullInt64  `db:"lurl_hash"` // 长链接的 MurmurHash3 值，用于高效索引
	}
)
...
func (m *defaultShortUrlMapModel) Insert(ctx context.Context, data *ShortUrlMap) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.CreateBy, data.IsDel, data.Lurl, data.Surl, data.LurlHash)
	return ret, err
}
...
func (m *defaultShortUrlMapModel) Update(ctx context.Context, newData *ShortUrlMap) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, newData.CreateBy, newData.IsDel, newData.Lurl, newData.Surl, newData.LurlHash, newData.Id)
	return err
}
